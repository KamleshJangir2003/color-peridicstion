...
<div class="card">
    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>#</th><th>User</th><th>Amount</th><th>Method</th><th>Txn ID</th><th>SS</th><th>Date</th><th>Status</th><th>Actions</th></tr>
            </thead>
    ...
    <div id="depPagination" style="display:flex;justify-content:center;gap:6px;margin-top:14px;flex-wrap:wrap;"></div>
</div>
...
<div class="modal-overlay" id="screenshotModal" onclick="closeModal('screenshotModal')">
    <div class="modal" onclick="event.stopPropagation()" style="max-width:400px;">
        ...
        <img id="screenshotImg" src="" style="width:100%;border-radius:10px;max-height:70vh;object-fit:contain;">
    </div>
</div>
...
@push('scripts')
<script>
let depFilter = 'pending', depPage = 1;
...
async function loadDeposits(page = 1) {
    ...
    document.getElementById('depositsTable').innerHTML = deps.length
        ? deps.map(d => `<tr>
            ...
            <td style="font-weight:800;color:var(--gold);">₹${d.amount}</td>
            <td style="font-size:12px;">${methodIcon[d.method] || ''} ${(d.method||'').toUpperCase()}</td>
            <td style="font-family:monospace;font-size:10px;color:var(--muted);max-width:80px;overflow:hidden;text-overflow:ellipsis;">${d.transaction_id || '--'}</td>
            <td>${d.screenshot ? `<button class="btn btn-sm btn-outline" onclick="viewScreenshot('/storage/${d.screenshot}')">📷</button>` : '<span style="color:var(--muted);font-size:11px;">—</span>'}</td>
            <td style="font-size:10px;color:var(--muted);">${new Date(d.created_at).toLocaleDateString('en-IN')}</td>
    ...
    if (data.last_page > 1) {
        ...
        for (let i = Math.max(1, depPage-2); i <= Math.min(data.last_page, depPage+2); i++) {
            html += `<button class="btn ${i===depPage?'btn-primary':'btn-outline'}" style="padding:5px 11px;" onclick="loadDeposits(${i})">${i}</button>`;
        }
...
async function rejectDeposit(id) {
    ...
    await AAPI(`/deposits/${id}/reject`, { method:'POST', body:JSON.stringify({ note }) });
